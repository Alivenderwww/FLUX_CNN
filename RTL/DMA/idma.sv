...
//     * wr_ptr 在 cfg_ifb_ring_words (= strip_rows * W_IN * cin_slices) 处 wrap
//     * 整体停止条件: rows_written == cfg_h_in_total
//   整图装得下 SRAM 时 strip_rows = H_IN, ring 不 wrap, 退化为原 batch 行为 =
//   IDMA 一次性写满整图然后 done (=0 ring wrap 的特例)。
//
...
module idma #(
    ...
    // 避免 unused-signal 警告 (byte_len 保留为 descriptor 兼容字段)
    ...
    typedef enum logic [2:0] {
        ...
        S_AR   = 3'd1,
        S_R    = 3'd2,
        S_WAIT = 3'd3,   // ring 满, 等 line_buffer 消费
        S_DONE = 3'd4
    } state_t;
    ...
    logic [15:0]             row_beats_remaining; // 当前行剩余 beats
    ...
    // ring_full 时抑制 AR，避免把 R 数据塞进刚装满的 ring
    ...
    always_comb begin
        ...
        case (state)
            ...
            S_AR   : begin
                if (ring_full)      state_next = S_WAIT;
    ...
    // row_beats_remaining: 每行开头 = W_IN, r_fire 减 1, 行末回 W_IN
    always_ff @(posedge clk) begin

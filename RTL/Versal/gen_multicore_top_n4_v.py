#!/usr/bin/env python3
"""Generate multicore_top_n4_v.v wrapper for N-core multicore (BD-friendly .v wrapper).

Each AXI port (csr_axil + m_axi) needs full X_INTERFACE_INFO attributes
per signal. Generate programmatically to avoid 500+ lines of boilerplate.

文件名保留 n4 后缀以兼容 BD reference; 改 N 后输出覆盖同一 .v, BD module_ref 不动.
...
CSR_AXIL_PARAM = (f'"MODE Slave, ADDR_WIDTH 12, DATA_WIDTH 32, PROTOCOL AXI4LITE, '
                  f'ID_WIDTH 0, HAS_BURST 0, HAS_LOCK 0, HAS_PROT 0, '
                  f'HAS_CACHE 0, HAS_QOS 0, HAS_REGION 0, HAS_WSTRB 1, HAS_BRESP 1, HAS_RRESP 1"')

MAXI_PARAM = (f'"MODE Master, PROTOCOL AXI4, ADDR_WIDTH 32, DATA_WIDTH 128, ID_WIDTH 4, '
              f'NUM_WRITE_OUTSTANDING 1, NUM_WRITE_THREADS 1, '
              f'NUM_READ_OUTSTANDING 1, NUM_READ_THREADS 1, READ_WRITE_MODE READ_WRITE, '
              f'HAS_BURST 1, HAS_LOCK 1, HAS_PROT 1, HAS_CACHE 1, HAS_QOS 1, HAS_REGION 0, '
              f'HAS_WSTRB 1, HAS_BRESP 1, HAS_RRESP 1, SUPPORTS_NARROW_BURST 1, '
              f'MAX_BURST_LENGTH 256, AWUSER_WIDTH 0, ARUSER_WIDTH 0, WUSER_WIDTH 0, '
              f'RUSER_WIDTH 0, BUSER_WIDTH 0"')
...
out_lines.append('')
out_lines.append('// =============================================================================')
out_lines.append(f'// multicore_top_n4_v.v  --  N={N} 多核 .v wrapper (auto-generated by gen_multicore_top_n4_v.py)')
out_lines.append(f'// 文件名保留 n4 后缀以兼容 BD reference (实际 N={N})')
out_lines.append('//')
out_lines.append(f'// 暴露 {N}×csr_axil (per-core 12-bit) + {N}×m_axi (per-core 128-bit data) 给 BD.')
out_lines.append(f'// BD 内 SmartConnect 处理 CIPS.M_AXI_FPD → csr_axil_N + {N}×m_axi → {N}×axi_bram_ctrl(URAM).')
out_lines.append('//')
out_lines.append(f'// 内部实例化 multicore_top_n4 SV 模块 (generate-for 实例化 {N} 个 core_top).')
out_lines.append('// =============================================================================')
out_lines.append('')
...
busif_list = ':'.join([f'csr_axil_{i}' for i in range(N)] + [f'm_axi_{i}' for i in range(N)])
out_lines.append(f'    (* X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF {busif_list}, ASSOCIATED_RESET rst_n" *)')
...
out_lines.append(f'    // 显式传 NUM_CORES={N} (SV 默认 2, wrapper 跟 N 同步)')
out_lines.append(f'    multicore_top_n4 #(.NUM_CORES({N})) u_mc_n4 (')

...
//
// 内部把 PARF 拆成 NUM_COL 个独立 parf_col 子模块, 每列各自一块 SRAM,
// 外壳负责 FILL/DRAIN 计数、握手、地址生成. 所有列共享 wr_addr / rd_addr / we,
// 每列只是 wdata 切片不同.
//
// DRAIN 与下一 tile FILL 的 first_round 重叠 (cross-tile overlap):
...
//
// 同步规则 2 (fill_tile_done vs drain_tile_done):
...
module parf_accum #(
    ...
    // ---- seed feedback to mac_array (bias) ----
    ...
    // =========================================================================
    // 每列 PSUM 存储实例化
    //   所有列共享同一 wr_addr / rd_addr / we, 只是 wdata 切片不同.
    // =========================================================================
    generate

...
CASES = [             #      c_in, c_out, k, h_in, w_in, stride, shift
    ...
    # --- K=3, Cin > HW_PE: chunked no-rounds (K*K*cin_slices > 32, kk <= 32) ---
    ("K=3 C64C16 66x118 s=1",   64,   16, 3,   68,  120,      1,     0),
    # --- K=7 chunked multi-round (kk=49 > 32) ---
    ("K=7 C8C8   62x114 s=1",    8,    8, 7,   68,  120,      1,     2),
    ("K=7 C16C16 62x114 s=1",   16,   16, 7,   68,  120,      1,     2),
    # --- K=5 small / stride ---
    ("K=5 C16C16 30x58  s=2",   16,   16, 5,   64,  120,      2,     1),
    # --- K=5 chunked no-rounds (cin_slices=2) ---
    ("K=5 C32C16 30x58  s=2",   32,   16, 5,   64,  120,      2,     1),
    # --- 大图暂注释（跑太久） ---

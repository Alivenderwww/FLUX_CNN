# =============================================================================
# tb_smc/run_chain_ip.tcl  --  SMC chain sim 用 axi_smc_4to4 IP (vs sim model)
#   用 +define+USE_AXI_SMC_IP 让 multicore_top_smc.sv 走 IP 实例化路径
#   IP 必须先用 vivado 生成 (cd c:\_Project\FLUX_CNN; vivado -mode batch -source Syn/gen_axi_smc_4to4.tcl)
#
# 跑法 (Windows PowerShell):
#   cd c:\_Project\FLUX_CNN\sim\tb_smc
#   vsim -c -do run_chain_ip.tcl
# =============================================================================
...
# Sanity check: axi_smc_4to4 IP must exist
...
# axi_smc_4to4 IP (新加的, 跟 axi_4to5 共用 axi_crossbar 库)
vlog -work xil_defaultlib \
    "$IP_SMC_DIR/sim/axi_smc_4to4.v"

# RTL (+define+USE_AXI_SMC_IP 切到 IP 路径)
vlog -sv -work work -mfcu -suppress 2902,13314 \
    +define+USE_AXI_SMC_IP \
    +incdir+../../RTL \
    ../../RTL/std_rf.sv \
    ../../RTL/sdp.sv \
    ../../RTL/sram_model.sv \
...
# 仿真. -L 加 axi_dm + axi_crossbar (axi_smc_4to4 跟 axi_4to5 都用 axi_crossbar IP)
vsim -c -voptargs="+acc" \
    ...
    -L unisims_ver \
    +CASE_DIR=cases/smc_resnet11 \
    work.tb_smc_chain xil_defaultlib.glbl
run -all
quit -f
